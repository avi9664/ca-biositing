"""
ETL Transform: Combustion Plants.

Transforms raw CSV data from the Combustion Plants dataset into a structured
format matching InfrastructureCombustionPlants. City and county columns are merged
...
def transform(
    ...
    """
    Transforms raw combustion plants data.
    ...
    Returns:
        A DataFrame ready for loading into infrastructure_combustion_plants.
    """
    try:
    ...
    for source_name in EXTRACT_SOURCES:
        ...
        coerced_df = coercion_mod.coerce_columns(
            cleaned_df,
            int_cols=["objectid", "yearloadbdt"],
            float_cols=["equivalent_generation", "np_mw", "cf", "latitude", "longitude"],
            datetime_cols=[],
        )
    ...
    # 6. Final Column Selection — matches InfrastructureCombustionPlants fields
    try:
        if etl_run_id:
