"""
ETL Transform: Ethanol Biorefineries.

Transforms raw CSV data from the Ethanol Biorefineries dataset into a structured
format matching InfrastructureEthanolBiorefineries. Address, city, and state columns
...
EXTRACT_SOURCES: List[str] = ["ethanol_biorefineries"]
...
def transform(
    ...
    for source_name in EXTRACT_SOURCES:
        ...
        coerced_df = coercion_mod.coerce_columns(
            cleaned_df,
            int_cols=["capacity_mgy", "production_mgy", "constr_exp"],
            float_cols=[],
            datetime_cols=[],
        )
    ...
    # 3. Geocode addresses — address, city, state are available
    ...
    rename_columns = {
        # No source column renames needed; all post-clean names match DB fields.
        # address_id is assigned directly by the geocode bridge below.
    }
