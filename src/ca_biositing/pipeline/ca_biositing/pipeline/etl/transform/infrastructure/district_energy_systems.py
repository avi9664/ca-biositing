"""
ETL Transform: District Energy Systems.

Transforms raw CSV data from the District Energy Systems dataset into a structured
format matching InfrastructureDistrictEnergySystems. City and state columns are merged
...
EXTRACT_SOURCES: List[str] = ["district_energy_systems"]
...
def transform(
    ...
    """
    Transforms raw district energy systems data.
    ...
    Returns:
        A DataFrame ready for loading into infrastructure_district_energy_systems.
    """
    try:
    ...
    # 3. Geocode addresses — city and state are the available location columns
    address_df, geoid_df = parse_addresses(
        combined_df,
        merge_columns=["city", "state"],
        lat="latitude",
        long="longitude",
    )
    ...
    rename_columns = {
        "cbgid": "cbg_id",
        "objectid": "object_id",
        "primaryfue": "primary_fuel",
        "secfuel": "secondary_fuel",
        "capst": "cap_st",
        "caphw": "cap_hw",
        "capcw": "cap_cw",
        "chpcgcap": "chpcg_cap",
    ...
    # 6. Final Column Selection — matches InfrastructureDistrictEnergySystems fields
    try:
        if etl_run_id:
