"""
ETL Transform: SAF and Renewable Diesel Plants.

Transforms raw CSV data from the SAF and Renewable Diesel Plants dataset into a
structured format matching InfrastructureSafAndRenewableDieselPlants. Address, city,
state, and country columns are merged and geocoded via parse_addresses to populate
LocationAddress and Place.
"""
...
EXTRACT_SOURCES: List[str] = ["saf_and_renewable_diesel_plants"]
...
def transform(
    ...
    """
    Transforms raw SAF and renewable diesel plants data.
    ...
    Returns:
        A DataFrame ready for loading into infrastructure_saf_and_renewable_diesel_plants.
    """
    try:
    ...
    # 3. Geocode addresses — address, city, state, country are available
    address_df, geoid_df = parse_addresses(
        combined_df,
        merge_columns=["address", "city", "state", "country"],
        lat="latitude",
        long="longitude",
    )
    ...
    # 6. Final Column Selection — matches InfrastructureSafAndRenewableDieselPlants fields
    try:
        if etl_run_id:
